                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.blog.pages.update', $page) }}" enctype="multipart/form-data">
                                ...
                                <div class="col-xs-12 col-md-8 offset-md-2">
                                    <div class="form-group photo-wrap">
                                        <div class="input-group">
                                            ...
                                            <input id="thumbnail" class="form-control" type="text" name="primary_media[url]" value="{{old('primary_media.url', $page->getPrimaryUrl())}}">

                                        </div>
                                        <div class="row">
                                            <div class="col-xs-12 col-md-4">
                                                <img id="holder" class="holder" src="{{old('primary_media.url', $page->getPrimaryUrl())}}">
                                            </div>
                                            <div class="col-xs-12 col-md-8">
                                                <div class="mb-3">
                                                    <label for="alt_logo" class="col-form-label text-md-right">{{ __('Alt') }}</label>
                                                    <input id="alt_logo" class="form-control" type="text" name="primary_media[alt]" value="{{old('primary_media.alt', $page->getPrimaryAlt())}}">
                                                </div>
                                                <div class="mb-3">
                                                    <label for="title_logo" class="col-form-label text-md-right">{{ __('Title') }}</label>
                                                    <input id="title_logo" class="form-control" type="text" name="primary_media[title]" value="{{old('primary_media.title',  $page->getPrimaryTitle())}}">
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @include('admin.partials.faq_block', ['faqs' => $faqs, 'enable_faq' => $page->enable_faq])
                                ...
                                <div class="col-xs-12 col-md-3">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group mb-3 date date-field edit-date">
                                            <input id="public_date" type="text" class="form-control{{ $errors->has('public_date') ? ' is-invalid' : '' }}" name="public_date" value="{{ old('public_date', $page->public_date->format('d/m/Y')) }}" >
                                        ...
                                        <div>
                                            <select id="user_id" name="user_id" class="form-control{{ $errors->has('user_id') ? ' is-invalid' : '' }}" required>
                                                @foreach ($users as $id => $username)
                                                    <option value="{{$id}}" {{selected(old('user_id', $page->user_id), $id)}}>{{$username}}</option>
                                                @endforeach
                                            </select>
